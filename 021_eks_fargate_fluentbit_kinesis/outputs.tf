...
output "kinesis_stream_name" {
  value       = local.outputs.kinesis_stream_name.value
  description = "Name of the Kinesis data stream every pod's log records are written to"
}
...
output "read_records_command" {
  value       = local.outputs.read_records_command.value
  description = "Command reading the records Fluent Bit has written into the stream's first shard"
}
