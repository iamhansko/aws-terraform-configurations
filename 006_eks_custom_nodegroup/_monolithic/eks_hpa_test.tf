# Generated from 006_eks_custom_nodegroup/eks_hpa_test.yaml by tools/cfn2tf.
...
MATCH_IMAGE_TAG=${data.aws_caller_identity.current.account_id}.dkr.ecr.${data.aws_region.current.region}.amazonaws.com/${aws_ecr_repository.ecr.name}:latest
...
eksctl utils associate-iam-oidc-provider --cluster ${aws_eks_cluster.eks_cluster.name} --region ${data.aws_region.current.region} --approve
...
mkdir -p /home/ec2-user/environment/cluster-autoscaler
eksctl create iamserviceaccount \
--name cluster-autoscaler \
--namespace kube-system \
--cluster ${aws_eks_cluster.eks_cluster.name} \
--attach-policy-arn "${aws_iam_policy.amazon_eks_cluster_autoscaler_policy.arn}" \
--approve --region ${data.aws_region.current.region} \
...
sed -i "s/<YOUR CLUSTER NAME>/${aws_eks_cluster.eks_cluster.name}/g" ~/environment/cluster-autoscaler/cluster-autoscaler-autodiscover.yaml
...
resource "aws_iam_policy" "amazon_eks_cluster_autoscaler_policy" {
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Sid      = "VisualEditor0"
      ...
      Sid      = "VisualEditor1"
      Effect   = "Allow"
      Action   = ["autoscaling:DescribeAutoScalingInstances", "autoscaling:DescribeAutoScalingGroups", "ec2:DescribeLaunchTemplateVersions", "autoscaling:DescribeTags", "autoscaling:DescribeLaunchConfigurations", "ec2:DescribeInstanceTypes"]
      Resource = "*"
    }]
  })
...
resource "aws_eks_node_group" "addon_node_group" {
  ...
  subnet_ids = [aws_subnet.private_subnet_a.id, aws_subnet.private_subnet_b.id, aws_subnet.private_subnet_c.id]
  launch_template {
    id      = aws_launch_template.addon_launch_template.id
    version = aws_launch_template.addon_launch_template.latest_version
  }
}
resource "aws_launch_template" "addon_launch_template" {
  ...
  tag_specifications {
    resource_type = "instance"
    tags = {
      Name = "addon-nodegroup-instance"
    }
  }
}
resource "aws_security_group" "eks_node_security_group" {
  description = "Security Group for EKS Node SSH Connection"
  name        = "eks-node-sg"
  vpc_id      = aws_vpc.vpc.id
  tags = {
    Name = "eks-node-sg"
...
resource "aws_vpc_security_group_ingress_rule" "eks_node_security_group_ingress" {
  security_group_id            = aws_security_group.eks_node_security_group.id
  ip_protocol                  = -1
  referenced_security_group_id = aws_security_group.eks_node_security_group.id
}
