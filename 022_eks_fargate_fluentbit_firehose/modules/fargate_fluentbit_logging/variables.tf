...
variable "pod_execution_role_name" {
  type        = string
  description = "Name of the Fargate pod execution IAM role the firehose:PutRecordBatch policy is attached to. Fluent Bit writes as this role, not as anything the pod carries"
...
variable "delivery_stream_name" {
  type        = string
  description = "Name of the Firehose delivery stream Fluent Bit writes to. Pass the firehose module's name output rather than restating it, so the ConfigMap cannot point at a stream that does not exist (rules.md B-5)"

  validation {
    condition     = can(regex("^[a-zA-Z0-9_.-]{1,64}$", var.delivery_stream_name))
    error_message = "delivery_stream_name must be 1-64 characters of letters, digits, underscores, dots or hyphens."
  }
}
variable "delivery_stream_arn" {
  type        = string
  description = "ARN of the same delivery stream, used to scope the IAM policy. Taken as a separate input rather than rebuilt from the name, because building an ARN by string concatenation is how a policy ends up granting nothing in a different partition or account"

  validation {
    condition     = can(regex("^arn:aws[a-z-]*:firehose:", var.delivery_stream_arn))
    error_message = "delivery_stream_arn must be a Firehose delivery stream ARN (arn:aws:firehose:...)."
  }
}
...
variable "policy_name" {
  type        = string
  default     = "FargatePodExecutionFirehosePolicy"
