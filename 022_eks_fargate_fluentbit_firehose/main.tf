...
# The sink: destination bucket, delivery stream, and the role the stream assumes
# to write into the bucket. One module because a delivery stream cannot exist
# without a role that can already reach its destination, and neither the role nor
# the bucket has a use outside this pipeline (rules.md C-2).
module "firehose_to_s3" {
  ...
  force_destroy              = var.destination_bucket_force_destroy
...
# built-in Fluent Bit log router, pointed at the delivery stream above, plus the
# PutRecordBatch permission on the pod execution role. The _monolithic template
# wrote the manifest to disk from user data and ran "kubectl apply -f" over SSM, so
...
module "fargate_fluentbit_logging" {
  ...
  # Name for the ConfigMap, ARN for the IAM policy. Both come from the firehose
  ...
  delivery_stream_name      = module.firehose_to_s3.name
  delivery_stream_arn       = module.firehose_to_s3.arn
  match                     = var.log_match
...
locals {
  ...
  outputs = {
    ...
    destination_bucket_name = {
      ...
      description = "Where Firehose delivers the log records, keyed by delivery time (YYYY/MM/DD/HH/). Nothing appears until the first buffering interval has elapsed"
    ...
    list_objects_command = {
      order       = 10
      title       = "5. List what Firehose delivered"
      description = "Run about two minutes after the pods start - one buffering interval plus delivery. An empty result before that is expected rather than a fault"
      value       = module.firehose_to_s3.list_objects_command
    }
    read_latest_object_command = {
      order       = 11
      title       = "6. Read the newest delivered object"
      description = "Prints the most recent file to stdout. Readable directly because the delivery stream writes uncompressed"
      value       = module.firehose_to_s3.read_latest_object_command
    }
    firehose_error_log_command = {
      order       = 12
      title       = "7. Read Firehose's delivery errors"
      description = "Records Firehose accepted but could not write to S3 are reported here, and the failed records themselves land under the bucket's error/ prefix. Empty output is the healthy case"
      value       = module.firehose_to_s3.delivery_log_tail_command
    }
    fluentbit_process_log_command = {
      order       = 13
      title       = "8. Read Fluent Bit's own log"
      description = "flb_log_cw is on, so the log router reports its own startup and errors here - including a PutRecordBatch the pod execution role was not allowed to make. The Fargate control plane names the group <cluster>-fluent-bit-logs and creates it as the pod execution role, so an empty result means the router either never started or lacks the CloudWatch permission"
      value       = "aws logs tail ${var.cluster_name}-fluent-bit-logs --since 15m"
    }
    demo_cleanup_command = {
      order       = 14
      title       = "9. Stop the demo workload"
      description = "Fargate bills per pod. The pods are Terraform resources now, so deleting them with kubectl only gets them recreated on the next apply - set create_demo_workload to false instead, which stops the per-pod charge and leaves the cluster and the log pipeline intact. Run this where the state is, not on the bastion. The delivery stream and bucket are Terraform resources, so terraform destroy removes them - including the delivered objects, since force_destroy is on"
