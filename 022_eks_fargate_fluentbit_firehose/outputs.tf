...
output "destination_bucket_name" {
  ...
  description = "Generated name of the S3 bucket Firehose delivers the log records to"
}
...
output "list_objects_command" {
  value       = local.outputs.list_objects_command.value
  description = "Command listing the objects Firehose has delivered to the bucket"
}
output "read_latest_object_command" {
  value       = local.outputs.read_latest_object_command.value
  description = "Command printing the most recently delivered object to stdout"
}
output "firehose_error_log_command" {
  value       = local.outputs.firehose_error_log_command.value
  description = "Command printing the delivery stream's own delivery errors. Empty output is the healthy case"
}
output "fluentbit_process_log_command" {
  value       = local.outputs.fluentbit_process_log_command.value
  description = "Command listing the CloudWatch log groups that may hold Fluent Bit's own process log, which is where a rejected PutRecordBatch is reported"
}
