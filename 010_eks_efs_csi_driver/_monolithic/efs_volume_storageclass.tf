# Generated from 010_eks_efs_csi_driver/efs_volume_storageclass.yaml by tools/cfn2tf.
...
variable "stack_name" {
  type        = string
  default     = "efs-volume-storageclass"
...
spec:
  ...
  resources:
    requests:
      storage: 5Gi' > /home/ec2-user/manifests/deployment.yaml
...
parameters:
  ...
  directoryPerms: "700"' > /home/ec2-user/manifests/storageclass.yaml
  ...
  "Statement": [
      {
          ...
          "Condition": {
              "StringEquals": {
                  "${element(split("//", aws_eks_cluster.eks_cluster.identity[0].oidc[0].issuer), 1)}:sub": "system:serviceaccount:kube-system:efs-csi-controller-sa",
...
resource "aws_eks_addon" "efs_csi_driver_addon" {
  ...
  cluster_name             = aws_eks_cluster.eks_cluster.name
  service_account_role_arn = aws_iam_role.efs_csi_driver_addon_iam_role.arn
}
