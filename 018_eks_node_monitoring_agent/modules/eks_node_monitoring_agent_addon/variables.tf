variable "cluster_name" {
  type        = string
  description = "Name of the EKS cluster to install the eks-node-monitoring-agent addon into"
...
variable "addon_version" {
  type        = string
  default     = null
  description = "Specific eks-node-monitoring-agent addon version (e.g. v1.3.8-eksbuild.2). When null, EKS picks the default version for the cluster's Kubernetes version"

  validation {
    condition     = var.addon_version == null || can(regex("^v[0-9]+\\.[0-9]+\\.[0-9]+-eksbuild\\.[0-9]+$", var.addon_version))
    error_message = "addon_version must look like v1.3.8-eksbuild.2, or null."
  }
}
