...
output "load_balancer_fetch_command" {
  ...
  description = "Command that fetches the dashboard through the load balancer, or null when service_type is not LoadBalancer. Built from load_balancer_hostname_command so both reference one source of truth (rules.md B-5). For an internal load balancer this only succeeds from inside the VPC, which is what the bastion is for"
}
output "container_port" {
  value       = var.container_port
  description = "Port the dashboard listens on inside the pod, and the Service's targetPort. Re-exposed because with nlb-target-type = ip the load balancer registers pod IPs on this port, so a caller opening the node-side path has to allow exactly it rather than the Service port (rules.md B-5)"
}
