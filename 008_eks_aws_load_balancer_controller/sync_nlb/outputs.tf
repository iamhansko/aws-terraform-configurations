...
output "synced_load_balancer_url" {
  ...
  description = "HTTP URL of the pre-created load balancer, available straight from Terraform state rather than only from 'kubectl get service'"
}
...
output "service_dns_name_command" {
  ...
  description = "Command that shows the Service. Its EXTERNAL-IP should be the pre-created load balancer, not a new one"
}
output "service_endpoint_command" {
  value       = local.outputs.service_endpoint_command.value
  description = "Command that prints the DNS name the controller actually attached to the Service, for comparing against synced_load_balancer_url"
}
output "service_fetch_command" {
  value       = local.outputs.service_fetch_command.value
  description = "Command that fetches the 2048 app through the NLB. For an internal load balancer this only succeeds from inside the VPC"
}
