...
variable "nlb_allow_inbound_from_anywhere" {
  type        = bool
  default     = true
  description = "Whether the internet-facing NLB accepts HTTP from 0.0.0.0/0. True by default because an internet-facing scheme nobody can reach is not much of a demo, but note this publishes an unauthenticated cluster dashboard: narrow it with nlb_ingress_cidr_blocks and set this false"
}
variable "nlb_ingress_cidr_blocks" {
  type        = list(string)
  default     = []
  description = "Specific CIDR blocks allowed inbound on the NLB, for narrowing access to known networks instead of 0.0.0.0/0. Applied in addition to nlb_allow_inbound_from_anywhere"

  validation {
    condition     = alltrue([for cidr in var.nlb_ingress_cidr_blocks : can(cidrhost(cidr, 0))])
    error_message = "nlb_ingress_cidr_blocks must contain valid IPv4 CIDR blocks."
  }
}
...
variable "synced_load_balancer_name" {
  type        = string
  default     = null
  description = "Optional fixed name for the pre-created load balancer the controller adopts. When null a unique name is generated, which avoids collisions when this project is deployed twice in one account"
