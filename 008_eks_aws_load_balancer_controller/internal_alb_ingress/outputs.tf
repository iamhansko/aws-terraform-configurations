...
output "ingress_describe_command" {
  value       = local.outputs.ingress_describe_command.value
...
output "ingress_fetch_command" {
  ...
  description = "Command that fetches the 2048 app through the ALB. The ALB is internal, so this only succeeds from inside the VPC - run it on the VS Code instance"
}
