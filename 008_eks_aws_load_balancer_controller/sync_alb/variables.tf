...
variable "alb_allow_inbound_from_anywhere" {
  type        = bool
  default     = true
  description = "Whether the internet-facing ALB accepts HTTP from 0.0.0.0/0. True by default because an internet-facing scheme that nobody can reach is not much of a demo; narrow it with alb_ingress_cidr_blocks and set this false to restrict who gets in"
}
variable "alb_ingress_cidr_blocks" {
  type        = list(string)
  default     = []
  description = "Specific CIDR blocks allowed inbound on the ALB, for narrowing access to known networks instead of 0.0.0.0/0. Applied in addition to alb_allow_inbound_from_anywhere"

  validation {
    condition     = alltrue([for cidr in var.alb_ingress_cidr_blocks : can(cidrhost(cidr, 0))])
    error_message = "alb_ingress_cidr_blocks must contain valid IPv4 CIDR blocks."
  }
}
