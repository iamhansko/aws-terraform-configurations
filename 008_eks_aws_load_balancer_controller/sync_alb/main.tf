...
# The ALB's frontend security group, assigned to the pre-created load balancer
# below at creation time and named in the Ingress annotation so the controller
# recognises the group as the one it should keep.
...
# The 2048 workload plus the Ingress. Declared before the load balancer below so
# its ingress_stack_tag output can drive that load balancer's tags.
locals {
  ...
  # which only exist when the Service is NodePort; this variant previously pinned
  # ClusterIP while still accepting instance, a combination the ALB cannot
  # register. Deriving it here means that can no longer be expressed.
  #
...
module "game_2048" {
  ...
  # destroy delete the Ingress before either disappears (rules.md D-4).
  depends_on = [module.network, module.eks_node_group, module.aws_load_balancer_controller]
}
# This is what the variant demonstrates: the ALB is created by Terraform rather
# than by the controller, carrying the three tags the controller stamps on the
# load balancers it owns. When the Ingress above is reconciled the controller
...
# ALB's ARN and DNS name up front, which a controller-created ALB never gives it.
module "synced_alb" {
...
locals {
  ...
  outputs = {
    ...
    alb_security_group_id = {
      ...
      description = "Group handed to the controller through the Ingress's alb.ingress.kubernetes.io/security-groups annotation, so the ALB gets a group whose inbound rules are visible in the plan"
    ...
    ingress_dns_name_command = {
      order       = 10
      title       = "1. Check the Ingress"
      description = "The ADDRESS column should fill in with the pre-created load balancer above rather than a new hostname. A different hostname means the stack tag did not match and the controller provisioned its own"
  ...
  # "2. Read the adopted ALB DNS name" above "1. Check the Ingress". Re-keying by
