...
module "alb_security_group" {
  ...
  description                 = "Frontend security group for the internet-facing ALB the controller provisions from the 2048 Ingress"
...
# internet-facing ALB in front of it.
locals {
...
module "game_2048" {
  ...
  ingress_annotations = {
    ...
    # the ALB would have no path to the pods.
...
locals {
  ...
  outputs = {
    ...
    alb_security_group_id = {
      ...
      description = "Group handed to the controller through the Ingress's alb.ingress.kubernetes.io/security-groups annotation. This variant's ALB is internet-facing, so this group is what decides who on the internet can reach it - and its rules are visible in the plan rather than invented by the controller"
    ...
    ingress_fetch_command = {
      ...
      description = "This variant's ALB is internet-facing, so unlike internal_alb_ingress the same hostname also opens the game in a browser on a laptop - subject to the frontend security group above"
