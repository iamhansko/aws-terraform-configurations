...
output "nlb_security_group_id" {
  ...
  description = "ID of the NLB's frontend security group, handed to the controller through the Service annotation. This variant's NLB is internet-facing, so this group decides who can reach it"
}
...
output "service_fetch_command" {
  ...
  description = "Command that fetches the dashboard through the NLB. This variant's NLB is internet-facing, so the same hostname also works from a browser"
}
