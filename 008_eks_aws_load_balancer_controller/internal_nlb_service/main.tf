...
#
# An internal NLB has no route in from the internet, so the only inbound rule is
# from the bastion's own security group: reach the dashboard by curling the NLB's
# DNS name from the VS Code instance.
module "nlb_security_group" {
  source = "./modules/load_balancer_security_group"

  vpc_id                      = module.network.vpc_id
  name                        = var.nlb_security_group_name
  description                 = "Frontend security group for the internal NLB the controller provisions from the kube-ops-view Service"
  allow_inbound_from_anywhere = false
...
module "kube_ops_view" {
  ...
  service_annotations = {
    ...
    # internal places the NLB in the private subnets with private addresses
    # only, so it is reachable from inside the VPC and nowhere else.
...
locals {
  ...
  outputs = {
    vscode_url = {
      order       = 1
      title       = "code-server"
      description = "Open the IDE here. Every command below is meant to be run from its terminal, because the NLB this variant creates is internal and unreachable from a laptop"
    ...
    nlb_security_group_id = {
      ...
      description = "Group handed to the controller through the Service's aws-load-balancer-security-groups annotation, so the NLB gets a group whose inbound rules are visible in the plan rather than one the controller invents"
    ...
    service_fetch_command = {
      ...
      description = "The NLB is internal, so it resolves to private addresses and is reachable from inside the VPC only. Run this from this instance - a browser on a laptop cannot reach it"
