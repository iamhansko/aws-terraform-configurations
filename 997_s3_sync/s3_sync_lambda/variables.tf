...
variable "force_destroy" {
  type        = bool
  default     = true
  description = "Whether terraform destroy may delete the bucket while it still holds objects. This is the native replacement for the empty_s3_bucket variant's Lambda custom resource"
}
...
variable "delete_on_destroy" {
  type        = bool
  default     = true
  description = "Whether terraform destroy invokes the function once more to remove the objects it uploaded, rather than relying on the bucket's force_destroy to sweep them"
}
