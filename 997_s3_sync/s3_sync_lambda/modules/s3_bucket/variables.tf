...
variable "force_destroy" {
  type        = bool
  default     = true
  description = "Whether terraform destroy may delete the bucket while it still holds objects. True is what makes the empty_s3_bucket custom resource unnecessary here; set false for a bucket holding anything worth keeping"
}
variable "sse_algorithm" {
  type        = string
  default     = "AES256"
  description = "Server-side encryption applied by default. Keep AES256 when the sync function is comparing ETags against file hashes: SSE-KMS makes the ETag stop being the content MD5, so every sync would re-upload every file"
