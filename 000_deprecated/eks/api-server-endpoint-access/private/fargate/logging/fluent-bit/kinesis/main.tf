# Generated from 000_deprecated/eks/api-server-endpoint-access/private/fargate/logging/fluent-bit/kinesis.yaml by tools/cfn2tf.
...
data:
  ...
  output.conf: |
    [OUTPUT]
      ...
      stream ${aws_kinesis_stream.kinesis_data_stream.name}" > /home/ec2-user/manifests/configmap.yaml
...
resource "aws_kinesis_stream" "kinesis_data_stream" {
  ...
  name = "${var.stack_name}-kinesis-data-stream"
}
# --- Outputs ---
