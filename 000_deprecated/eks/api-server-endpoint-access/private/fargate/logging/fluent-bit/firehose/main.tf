# Generated from 000_deprecated/eks/api-server-endpoint-access/private/fargate/logging/fluent-bit/firehose.yaml by tools/cfn2tf.
...
resource "aws_iam_role_policy" "firehose_iam_role" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = ["arn:aws:s3:::${aws_s3_bucket.destination_s3_bucket.id}", "arn:aws:s3:::${aws_s3_bucket.destination_s3_bucket.id}/*"]
      }, {
      Sid      = "KinesisAccess"
      Effect   = "Allow"
      ...
      Condition = {
        ...
        StringLike = {
          "kms:EncryptionContext:aws:s3:arn" = "arn:aws:s3:::${aws_s3_bucket.destination_s3_bucket.id}/*"
        }
      }
      }, {
      ...
      Resource = ["arn:aws:logs:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:log-group:*:log-stream:*"]
      }, {
      Sid      = "LambdaAccess"
      Effect   = "Allow"
...
data:
  output.conf: |
    [OUTPUT]
      ...
      delivery_stream ${aws_kinesis_firehose_delivery_stream.firehose.id}" > /home/ec2-user/manifests/configmap.yaml
