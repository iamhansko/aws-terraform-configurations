...
variable "nodejs_version" {
  ...
  validation {
    ...
    error_message = "nodejs_version must be a Node.js major version number (e.g. 20)."
  }
}
...
variable "load_balancer_dns_name" {
  type        = string
  description = "DNS name of the load balancer, written to the app's .env file as ELB_DNS_NAME"

  validation {
    condition     = length(var.load_balancer_dns_name) > 0
    error_message = "load_balancer_dns_name must not be empty."
  }
}
...
variable "additional_user_data" {
  type        = string
  default     = ""
  description = "Additional shell script content appended after the .env file is written and before npm run start:game"
}
