...
variable "create_namespace" {
  type        = bool
  default     = false
  description = "Whether to create the namespace this profile selects. A selector does not create the namespace it names - EKS accepts a profile pointing at one that does not exist - so for a namespace the project introduces this has to be true, or the first 'kubectl run -n <namespace>' fails with NotFound. Opt-in rather than on by default because a profile selecting a namespace Kubernetes ships (kube-system, default) must leave it alone: managing one of those here would put it in Terraform state and have terraform destroy try to delete it"
}
variable "subnet_ids" {
  type        = list(string)
