...
variable "opensearch_domain_name" {
  ...
  description = "Name of the OpenSearch domain Fluent Bit writes pod logs into. The _monolithic template derived this from the CloudFormation stack name; it is a plain variable here (rules.md B-3)"

  validation {
    condition     = can(regex("^[a-z][a-z0-9-]{2,27}$", var.opensearch_domain_name))
    error_message = "opensearch_domain_name must be 3-28 characters, start with a lowercase letter, and contain only lowercase letters, digits and hyphens."
  }
}
variable "opensearch_instance_type" {
  type        = string
  default     = "m5.large.search"
  description = "Data node instance type. By far the largest running cost in this project: the domain bills continuously, unlike the Fargate pods which bill only while they exist"

  validation {
    condition     = can(regex("\\.search$", var.opensearch_instance_type))
    error_message = "opensearch_instance_type must be an OpenSearch instance type ending in .search."
  }
}
variable "opensearch_master_user_name" {
  type        = string
  default     = "admin"
  description = "OpenSearch Dashboards login. A fine-grained access control user in the domain's internal database, not an IAM identity"

  validation {
    condition     = length(var.opensearch_master_user_name) > 0
    error_message = "opensearch_master_user_name must not be empty."
  }
}
variable "opensearch_master_user_password" {
  type        = string
  sensitive   = true
  description = "Password for the OpenSearch master user. No default on purpose: the _monolithic template shipped adminPassword1234! as a plain default, which put a working credential into every plan, state file and console page. Pass it with TF_VAR_opensearch_master_user_password or a tfvars file kept out of git"

  validation {
    condition     = length(var.opensearch_master_user_password) >= 8
    error_message = "opensearch_master_user_password must be at least 8 characters."
  }
}
variable "opensearch_index_name" {
  type        = string
  default     = "web"
  description = "Index Fluent Bit writes every record into. One index for the whole cluster, as the _monolithic template had it"

  validation {
    condition     = can(regex("^[a-z0-9][a-z0-9._-]*$", var.opensearch_index_name))
    error_message = "opensearch_index_name must be lowercase and start with a letter or digit."
  }
}
variable "include_kubernetes_filter" {
  type        = bool
  default     = false
  description = "Whether Fluent Bit attaches pod metadata (name, namespace, labels) to each record. False reproduces the _monolithic template for this variant, whose ConfigMap had no filters at all - so every document in the index holds a bare log line with nothing identifying the pod that produced it"
}
variable "log_match" {
  type        = string
  default     = "*"
...
variable "opensearch_log_writer_role_name" {
  type        = string
  default     = "fargate-log-writer"
  description = "Name of the OpenSearch security role the Fargate pod execution role is mapped onto. Fine-grained access control is a separate authorization layer from IAM, so this mapping is required on top of the domain's access policy - without it every write is rejected with 403 and the domain simply stays empty"

  validation {
    condition     = can(regex("^[a-zA-Z0-9_.-]{1,64}$", var.opensearch_log_writer_role_name))
    error_message = "opensearch_log_writer_role_name must be 1-64 characters of letters, digits, underscores, dots or hyphens."
  }
}
variable "flb_log_cw" {
  type        = bool
  default     = true
  description = "Whether Fluent Bit ships its own process log to CloudWatch. True rather than the _monolithic template's omitted key, which is a deliberate departure: this is the only channel that reports a rejected write, and with it off a permissions problem at the OpenSearch end looks identical to the log router never having started. Setting it true also adds the CloudWatch statement to the pod execution role's policy, since the router needs to be able to write it"
}
