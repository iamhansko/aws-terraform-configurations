...
# built-in Fluent Bit log router, plus the CloudWatch permission on the pod
# execution role. The _monolithic template wrote both manifests to disk from user
# data and ran "kubectl apply -f" over SSM, so nothing connected the rendered
# region and log group names to Terraform afterwards; here they are resource
# arguments (rules.md E-1/E-2).
...
locals {
  ...
  outputs = {
    ...
    log_group_names = {
      ...
      description = "Where the two workloads' logs land, one group each. They do not exist until a matching pod has logged something - Fluent Bit creates them, not Terraform"
      value       = join(" ", module.fargate_fluentbit_logging.log_group_names)
    }
    ...
    log_tail_command = {
      order       = 10
      title       = "5. Tail the logs from CloudWatch"
      description = "Run once each pod has been up for a minute. An empty result usually means the pod's app label matches no OUTPUT block"
      value       = "aws logs tail ${module.fargate_fluentbit_logging.log_group_names[0]} --follow"
    }
    demo_cleanup_command = {
      order       = 11
      title       = "6. Stop the demo workload"
      description = "Fargate bills per pod. The pods are Terraform resources now, so deleting them with kubectl only gets them recreated on the next apply - set create_demo_workload to false instead, which stops the per-pod charge and leaves the cluster and the log pipeline intact. Run this where the state is, not on the bastion. The log groups stay behind either way - Fluent Bit created them, so terraform destroy does not remove them"
