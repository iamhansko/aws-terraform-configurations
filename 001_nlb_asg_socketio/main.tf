...
module "app_asg" {
  ...
  load_balancer_security_group_id = module.network_load_balancer.security_group_id
  target_group_arns               = [module.network_load_balancer.target_group_arn]
  min_size                        = var.app_min_size
  max_size                        = var.app_max_size
  prefix                          = var.prefix

  depends_on = [module.network, module.bastion_ec2, module.network_load_balancer]
}
